export const Rating = ({ rate }) => {
  // convert to array and fill it with value of rate
  const stars = Array(rate).fill(0);
  console.log(stars);

  return (
    <div class="flex gap-0.5 pt-4">
      {stars.map((_, index) => (
        <svg
          key={index}
          class="h-6 w-6 shrink-0 fill-amber-400"
          viewBox="0 0 256 256"
        >
          <path d="M239.2 97.4A16.4 16.4.0 00224.6 86l-59.4-4.1-22-55.5A16.4 16.4.0 00128 16h0a16.4 16.4.0 00-15.2 10.4L90.4 82.2 31.4 86A16.5 16.5.0 0016.8 97.4 16.8 16.8.0 0022 115.5l45.4 38.4L53.9 207a18.5 18.5.0 007 19.6 18 18 0 0020.1.6l46.9-29.7h.2l50.5 31.9a16.1 16.1.0 008.7 2.6 16.5 16.5.0 0015.8-20.8l-14.3-58.1L234 115.5A16.8 16.8.0 00239.2 97.4z"></path>
        </svg>
      ))}
      <div className="px-4 text-[18px] font-semibold text-slate-500 cursor-pointer">
        <span className="transition-all hover:text-slate-900 hover:border-b hover:border-b-black">4.2 of 10 reviews</span>
      </div>
    </div>
  );
};
